...
"""Base launcher functionality and in-process channels."""
...
class PassiveSnapshotChannel(messages.SnapshotChannel):
...
class PassiveEventChannel(messages.EventChannel):
...
def make_in_process_endpoints() -> (
  ...
  return endpoints.make_endpoints(
      s2v_snapshot=PassiveSnapshotChannel(),
      s2v_events=PassiveEventChannel(),
      v2s_events=PassiveEventChannel(),
      v2s_snapshot=PassiveSnapshotChannel(),
  )
...
def launch_in_thread(
  ...
  viewer_endpoint, sim_endpoint = make_in_process_endpoints()
